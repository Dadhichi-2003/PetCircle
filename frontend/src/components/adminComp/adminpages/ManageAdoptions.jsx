...
const ManageAdoptions = () => {
  ...
  const fetchAdoptions = async () => {
    try {
      setLoading(true);
      // Fetch available pets using the `/adoptions` API endpoint
      const res = await axios.get("/admin/adoptions");
      const adoptions = res.data.pets;

      const grouped = {};
      adoptions.forEach((adopt) => {
        const userId = adopt.owner._id;
        if (!grouped[userId]) {
          grouped[userId] = {
            user: adopt.owner,
            adoptions: [],
  ...
  useEffect(() => {
    ...
    Object.entries(groupedAdoptions).forEach(([userId, data]) => {
      const filtered = data.adoptions.filter((adoption) => {
        const petName = adoption.petname?.toLowerCase() || "";
  ...
  const handleDelete = async () => {
    try {
      await Promise.all(
        selectedAdoptions.map((id) =>
          axios.delete(`/admin/adoption/${id}`) // Delete adoption request using the deleteAdoption API
        )
      );
  ...
  return (
    <Box p={4}>
            ...
            return (
              <Box key={userId} mb={5}>
                    ...
                    <TableBody>
                      {paginated.map((adoption) => (
                        <TableRow key={adoption._id} hover>
                          ...
                          <TableCell>
                            <img
                              src={adoption.profilePic || "/default-pet.jpg"}
                          ...
                          <TableCell>{adoption.medicalHistory.join(", ")}</TableCell>
                          <TableCell>
                ...
                <TablePagination
                  ...
                  page={currentPage}
                  onPageChange={(_, newPage) => handlePageChange(userId, newPage)}
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>{bulkDelete ? "Delete Selected Adoption Requests" : "Delete Adoption Request"}</DialogTitle>
        <DialogContent>
          <DialogContentText>
            {bulkDelete
              ? "Are you sure you want to delete the selected adoption requests?"
        ...
        <DialogActions>
          ...
          <Button
            ...
            variant="contained"
          >
            {bulkDelete ? "Delete Selected" : "Delete"}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
