...
const ManagePosts = () => {
  ...
  const applyFilters = (postList = posts) => {
    ...
    if (search.trim()) {
      filtered = filtered.filter((p) =>
        p.postedBy?.username?.toLowerCase().includes(search.toLowerCase())
      );
    }
  ...
  const groupByUser = () => {
    ...
    filteredPosts.forEach((post) => {
      const username = post.postedBy?.username || "Unknown";
      ...
      {Object.entries(groupedPosts).map(([username, userPosts]) => (
        <Box key={username} mb={4}>
          <Box display="flex" alignItems="center" gap={2} mb={2}>
            {userPosts[0]?.postedBy?.profilePic && (
              <img
                src={userPosts[0].postedBy.profilePic}
                alt="Profile"
                style={{
                  width: 50,
                  height: 50,
            ...
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                {userPosts[0]?.postedBy?.email || "No email"}
              </Typography>
            </Box>
          </Box>
